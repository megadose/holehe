...
from holehe.defense import RateLimitedException, RequestPacer
from holehe.proxy import ClientManager, ProxyRotator, SiteClient
...
def list_available_sites(modules):
    ...
    print("\n" + "=" * 65)
    print(f" Holehe v{__version__} - Available Modules ({len(websites)} total)")
    print("=" * 65)
...
def check_update():
    """Check for holehe updates safely without crashing if unreachable."""
    try:
        check_version = httpx.get("https://pypi.org/pypi/holehe/json", timeout=3.0)
        if check_version.status_code == 200:
            ...
            if latest_v != __version__:
                ...
                p.communicate()
                print("Holehe has been updated to the latest version. Please restart.")
                exit(0)
    except Exception:
        pass
...
def export_csv(data, args, email):
    ...
    if getattr(args, "csvoutput", False):
        ...
        name_file = f"holehe_{round(timestamp)}_{email}_results.csv"
        all_keys = [
